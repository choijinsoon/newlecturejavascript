...
export default class GameCanvas{
    ...
    run(){ 
        ...
        if(this.enemyInterval == 15)  {
            var rx = Math.floor(Math.random()*this.obj.width);  
        ...
        for (var d of this.defenders){
            if (d.x > this.player.x-40 && d.x < this.player.x+40 &&
        ...
        for (var r of this.referees){
            if (r.x > this.player.x-40 && r.x < this.player.x+40 &&
                r.y > this.player.y-15 && r.y < this.player.y+15){ 
        ...
        for(var c of this.cards){
            if(this.player.y > c.y-0 && this.player.y < c.y+80){ 
